...
- Preserve the current Settings polish contract: the initial window should fit the full form on a normal display, numeric duration/threshold controls should remain manually editable through text entry plus stepper affordances, and the break-message field should show the active value directly without an English placeholder.
- `launchAtLoginEnabled` in launch-loaded `config.json` is the desired-state control for Launch at Login; the shipped Settings window must only display that state as a disabled read-only row while runtime/config updates still reconcile through the existing sync path. Treat approval/registration/unregistration problems as non-fatal warnings, and do not document or assume guaranteed registration on unsigned builds.
...
- Session inactive/lock suppression is always-on: ordinary screen lock uses an isolated best-effort distributed screen-lock observer plus current-state/startup sampling, while `NSWorkspace` session active/inactive notifications remain the session-switch path. Both feed the same away semantics: active work resets to fresh work, active rest closes silently into fresh work, paused reminders remain paused, locked time does not advance timers, and optional tray timer mode may show bounded `Away` during active suppression.
