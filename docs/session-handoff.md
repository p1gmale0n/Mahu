...
## 2026-06-11 / Settings miniaturized quit guard review fix

🏁 Session Handoff:
- Status: Done
- Key Decisions: Fix only the newly verified major lifecycle defect from this review round. `SettingsWindowController.shouldAllowApplicationTermination()` now treats a miniaturized Settings window as an active termination guard participant, and if quit is vetoed by a close-triggered save warning it restores and foregrounds the retained window so the inline warning is actually visible. Leave fully closed/non-visible retained windows non-blocking, because they are not an active editing session.
...
- Friction/CDD: The external review gate still expects lint evidence, but this repo still has neither a tracked lint target nor an installed `swiftlint`, so deterministic proof remains XCTest/build/package plus diff hygiene. The bug also lived in AppKit lifecycle state rather than pure model logic, so the regression test has to exercise real window miniaturization semantics instead of pretending view-model coverage is enough.
- Next Steps: Let the external review loop rerun from the next fix commit; if lint remains mandatory, add a repo-owned lint target or install `swiftlint` in the environment; keep a manual quit-path check explicit on real hardware because live AppKit activation timing can still differ slightly from hosted XCTest.

## 2026-06-11 / Settings close and quit review fix

🏁 Session Handoff:
- Status: Done
- Key Decisions: Fix only the two newly verified major lifecycle defects from this review round. `SettingsWindowController` now treats any close-triggered draft commit that still leaves a save warning active as an immediate close veto, even when the warning text was already visible and `saveWarningRevision` does not advance. `AppDelegate` also now reuses the same guard from `applicationShouldTerminate(_:)`, because the status-menu `Quit` path must not rely on implicit AppKit termination behavior to run `windowShouldClose(_:)`.
- Validation: `xcodebuild test -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO -only-testing:MahuTests/SettingsWindowControllerTests -only-testing:MahuTests/AppDelegateCompositionTests`; `xcodebuild test -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO`; `xcodebuild build -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO`; `make build`; `git diff --check`; `command -v swiftlint` (not installed); `rg -n '(^lint:|swiftlint|make lint)' Makefile README.md docs -S` (history/docs mentions only; no repo-owned lint target).
- Friction/CDD: The external review gate still assumes lint evidence, but this repo still has neither a tracked lint target nor an installed `swiftlint`, so deterministic proof remains XCTest/build/package plus diff hygiene. The confirmed bugs also sat at the boundary between retained AppKit window lifecycle and SwiftUI-local draft state, so truthful regression coverage had to stay presenter/composition-level instead of pretending model-only tests were enough.
- Next Steps: Let the external review loop rerun from the next fix commit; if lint remains mandatory, add a repo-owned lint target or install `swiftlint` in the environment; keep manual real-window verification explicit for close/quit warning visibility because XCTest still cannot prove every live AppKit timing nuance on hardware.

## 2026-06-11 / Settings close warning and fractional mapping review fix

🏁 Session Handoff:
- Status: Done
- Key Decisions: Fix only the two newly verified major review findings. The retained Settings window now treats save warnings as close-blocking only until they have remained visible through the next main-queue turn, which covers AppKit's real `didEndEditing -> windowShouldClose(_:)` order without turning old warnings into a permanent close trap. The Settings timer display mapping and edit-time normalization also moved into `SettingsValueMapper`, both to honor the documented nearest-supported-value contract for fractional raw config values and to pull `SettingsViewModel.swift` back under the local growth threshold.
- Validation: `xcodebuild test -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO -only-testing:MahuTests/SettingsWindowControllerTests -only-testing:MahuTests/SettingsLegacyDisplayMappingTests -only-testing:MahuTests/SettingsViewModelTests -only-testing:MahuTests/SettingsNumericStepperFieldTests`; `xcodebuild test -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO`; `xcodebuild build -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO`; `make build`; `git diff --check`; `command -v swiftlint` (not installed); `rg -n '(^lint:|swiftlint|make lint)' Makefile README.md docs -S` (history/docs mentions only; no repo-owned lint target).
- Friction/CDD: The review gate still expects lint evidence, but this repo still has neither a tracked lint target nor an installed `swiftlint`, so deterministic proof remains XCTest/build/package plus diff hygiene. The close-path bug also sat behind real AppKit ordering rather than pure Swift logic, so I had to verify it with a local `performClose(nil)` experiment before changing the presenter contract.
- Next Steps: Let the external review loop rerun from the next fix commit; if lint remains mandatory, add a repo-owned lint target or install `swiftlint` in the environment; keep manual real-window verification explicit for menu-bar behavior and inline warning rendering because XCTest still cannot prove the exact live AppKit timing on hardware.

## 2026-06-11 / Settings warning aggregation and retry review fix

🏁 Session Handoff:
- Status: Done
- Key Decisions: Fix only the two newly verified major Settings persistence defects from this review round. Repeated no-op commits for non-message fields now retry `config.json` persistence if the previous accepted runtime update failed to save, and the shared Settings footer now aggregates oversized break-message draft warnings with unrelated generic save failures instead of hiding one behind the other. Keep the current close-path gate unchanged in this pass because the remaining report there is a narrower UX edge case, not a verified major persistence break.
- Validation: `xcodebuild test -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO -only-testing:MahuTests/SettingsPersistenceWarningTests -only-testing:MahuTests/SettingsBreakOverlayMessageTests -only-testing:MahuTests/SettingsViewModelTests`; `xcodebuild test -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO`; `xcodebuild build -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO`; `make build`; `git diff --check`; `command -v swiftlint` (not installed); `rg -n '(^lint:|swiftlint|make lint)' Makefile README.md docs -S` (history/docs mentions only; no repo-owned lint target).
- Friction/CDD: The external review loop still treats lint as mandatory evidence, but this repo still has neither `swiftlint` nor a tracked lint target, so deterministic proof remains XCTest/build/package plus diff hygiene. `SettingsViewModel.swift` is also already above the local readability threshold, so this fix kept new warning-composition logic in a dedicated helper file and moved new regression coverage into a dedicated test file instead of growing the existing view-model test blob further.
- Next Steps: Let the external review loop rerun from the next fix commit; if lint remains mandatory, add a repo-owned lint target or install `swiftlint` in the environment; if a later review still insists on the close-veto edge case, handle it as a separate narrow presenter-path change with dedicated `SettingsWindowController` coverage.

## 2026-06-11 / Settings Polish Review Hardening

🏁 Session Handoff:
- Status: Done
- Key Decisions: Split legacy Settings display mapping from edit-time normalization so the UI now shows the nearest supported timer values instead of always rounding upward, while also making huge idle-away threshold mapping overflow-safe. Preflight Settings commits against the same strict-JSON size limit as `ConfigStore.save(_:)` so oversized break-message drafts stay local with a warning instead of becoming runtime-only state that disappears on restart. Sync README and the active polish plan to the actual close-path contract, and remove the redundant `SettingsNumericStepperControl` wrapper instead of carrying a pure pass-through type.
...
- Friction/CDD: The external review prompt still treats lint as mandatory evidence, but this repo still has neither `swiftlint` nor a tracked lint target, so deterministic proof remains XCTest/build/package plus diff hygiene. Hosted AppKit assertions are also still brittle at the SwiftUI bridge seam, so the verified fixes leaned on stronger model/composition coverage and targeted hosted checks rather than pretending every Settings footer/toggle state is stably introspectable in this environment.
- Next Steps: Let the external review loop rerun from the next fix commit; if lint must remain mandatory, add a repo-owned lint target or install `swiftlint` in the environment; keep manual checks explicit for real menu-bar rendering, close-triggered Settings warning visibility, and other AppKit-hosted behavior that XCTest still cannot prove robustly.

## 2026-06-10 / Settings Review Pass Numeric Draft Close Coordination

🏁 Session Handoff:
- Status: Done
- Key Decisions: Fix only the two newly verified major findings from this review pass. Numeric Settings fields now treat blur/submit as a real commit only after an actual text edit, which preserves the shipped legacy-raw contract during pure focus churn. The retained Settings window now runs one shared close-path draft committer for numeric and break-message controls before deciding whether close is allowed, so close-triggered numeric saves surface the same warning behavior as break-message drafts. Reject the repeated-close warning report as a deliberate existing contract once a warning is already visible, and reject the main-thread save plus oversized `StatusItemController` findings as non-major for this pass.
- Validation: `xcodebuild test -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO -only-testing:MahuTests/SettingsNumericStepperFieldTests -only-testing:MahuTests/SettingsWindowControllerTests`; `xcodebuild test -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO`; `xcodebuild build -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO`; `make build`; `git diff --check`; `command -v swiftlint` (not installed); `rg -n '(^lint:|swiftlint|make lint)' Makefile README.md docs -S` (history/docs mentions only; no repo-owned lint target).
- Friction/CDD: The review gate still assumes lint proof, but this repo still has neither a tracked lint target nor an installed `swiftlint`, so deterministic evidence remains XCTest/build/package plus diff hygiene. Draft-state regressions also sit at the SwiftUI-focus plus retained-AppKit-window seam, so model-only tests are not enough; presenter-level close-path coverage is still required.
- Next Steps: Let the external review loop rerun from the next fix commit; if lint remains mandatory, add a repo-owned lint target or install `swiftlint` in the environment; keep manual menu-bar and real Settings-window behavior checks explicit because this pass only tightened deterministic draft-commit semantics.

## 2026-06-10 / Settings Window Second Review Pass Close Warning Visibility Fix

🏁 Session Handoff:
- Status: Done
- Key Decisions: Treat the only newly verified major finding as a retained-window AppKit lifecycle bug: close-triggered break-message commits must happen in `windowShouldClose(_:)`, not `windowWillClose(_:)`, so a failed save can keep the Settings window open long enough for the inline warning to render. Keep closing non-blocking when a warning was already visible from earlier edits and no pending break-message draft still needs a close-path commit. Reject the other agent findings as either documented preserve-raw Settings behavior, an already deliberate synchronous `persist immediately` contract, or non-major maintainability feedback.
- Validation: `xcodebuild test -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO -only-testing:MahuTests/SettingsWindowControllerTests -only-testing:MahuTests/SettingsViewModelTests`; `xcodebuild test -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO`; `xcodebuild build -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO`; `make build`; `git diff --check`; `command -v swiftlint` (not installed); `rg -n '(^lint:|swiftlint|make lint)' Makefile README.md docs -S` (history/docs mentions only; no repo-owned lint target).
- Friction/CDD: The review gate still expects lint evidence, but this repo still has neither a tracked lint target nor an installed `swiftlint`, so deterministic proof remains XCTest/build/package plus diff hygiene. The retained Settings window also keeps hiding bugs at the AppKit lifecycle seam rather than in the SwiftUI form body, so branch-safe regression coverage needs presenter-level tests, not just hosted-view assertions.
- Next Steps: Let the external review loop rerun from the next fix commit; if lint remains mandatory, add a repo-owned lint target or install `swiftlint` in the environment; keep manual menu-bar/window behavior checks explicit because this pass only tightened deterministic close-path warning visibility.

## 2026-06-10 / Settings Window Review Findings Fixes

🏁 Session Handoff:
- Status: Done
- Key Decisions: Treat the confirmed review findings as three shipped-contract fixes: keep break-overlay message edits as local draft state until explicit commit paths (`Return`, focus loss, or Settings window close) instead of persisting on every keystroke; retry strict-JSON persistence on a repeated explicit commit when the previous save failed even if the normalized message text is unchanged; and enforce the Settings window minimum through `window.contentMinSize` so the full form stays reachable by content sizing rules. Add deterministic coverage around real `ConfigStore` persistence wiring, the hosted numeric-stepper field, and break-message commit/retry semantics, while rejecting the remaining subjective over-engineering reports as non-defects.
- Validation: `xcodebuild test -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO -only-testing:MahuTests/SettingsBreakOverlayMessageTests -only-testing:MahuTests/SettingsViewModelTests -only-testing:MahuTests/SettingsRuntimeIntegrationTests -only-testing:MahuTests/SettingsWindowControllerTests -only-testing:MahuTests/AppDelegateCompositionTests`; `xcodebuild test -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO -only-testing:MahuTests/SettingsNumericStepperFieldTests`; `xcodebuild test -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO`; `xcodebuild build -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO`; `make build`; `git diff --check`; `command -v swiftlint` (not installed); `rg -n '(^lint:|swiftlint|make lint)' Makefile README.md docs -S` (history/docs mentions only; no repo-owned lint target).
- Friction/CDD: The review gate still expects lint evidence, but this repo still has neither a tracked lint target nor an installed `swiftlint`, so deterministic proof remains XCTest/build/package plus diff hygiene. Hosted AppKit/SwiftUI verification is also easy to fake with body-shape assertions, so this pass replaced brittle pseudo-tests with checks that instantiate the real controls or the real `ConfigStore` wiring.
- Next Steps: Let the external review loop rerun from the new fix commit; if lint remains mandatory, add a repo-owned lint command or install `swiftlint` in the environment; keep manual menu-bar/window behavior and live break-overlay text checks explicit because this pass only tightened deterministic Settings/runtime semantics.
...
## 2026-06-11 / Settings Rejected Draft Review Fix

🏁 Session Handoff:
- Status: Done
- Key Decisions: Treat the only newly verified major finding as a rejected-draft state bug inside `SettingsViewModel`, not as a full close-flow redesign. Keep the oversized break-message warning separate from generic config-save failures and preserve the rejected draft text across unrelated successful Settings saves until the user shortens it into a persistable range or cancels back to the committed value.
- Validation: `xcodebuild test -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO -only-testing:MahuTests/SettingsBreakOverlayMessageTests -only-testing:MahuTests/SettingsViewModelTests -only-testing:MahuTests/SettingsWindowControllerTests`; `xcodebuild test -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO`; `xcodebuild build -project "Mahu.xcodeproj" -scheme "Mahu" -destination "platform=macOS" CODE_SIGNING_ALLOWED=NO`; `make build`; `git diff --check`; `command -v swiftlint` (not installed); `rg -n '(^lint:|swiftlint|make lint)' Makefile README.md docs -S` (no repo-owned lint target found).
- Friction/CDD: The external review loop still expects lint proof, but this repo still has neither a tracked lint target nor an installed `swiftlint`, so deterministic validation remains XCTest/build/package plus explicit lint-tool absence. The retained Settings window also continues to concentrate subtle bugs at the boundary between local SwiftUI draft state and model-level refreshes, so future fixes should keep adding focused regression coverage for cross-field interactions instead of only single-control tests.
- Next Steps: Let the external review loop rerun from the fix commit; if lint remains mandatory, add a repo-owned lint command or install `swiftlint` in the environment; keep manual real-window checks explicit because this pass tightened only the deterministic rejected-draft persistence path.
